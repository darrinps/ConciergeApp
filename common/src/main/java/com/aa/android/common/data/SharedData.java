...
public abstract class SharedData implements Parcelable
{
     ...
     */
    public static <T extends SharedData> T fromDataItem(Class<T> clazz, DataItem dataItem)
    {
        if (dataItem == null)
        {
            throw new IllegalArgumentException("the data item must not be null");
        }
        return SharedData.fromDataMapItem(clazz, DataMapItem.fromDataItem(dataItem));
    }
     ...
     *
     * @throws IllegalArgumentException
     *         if the {@code dataMapItem is null} or
     *         if the {@code CREATOR} field cannot be found
     * @throws IllegalStateException
     *         if the {@code CREATOR} object cannot be retrieved
     */
    public static <T extends SharedData> T fromDataMapItem(Class<T> clazz, DataMapItem dataMapItem)
    {
        if (dataMapItem == null)
        {
        ...
        try
        {
            ...
            return CREATOR.createFromDataMap(map);
        }
     ...
     * field that generates instances of your SharedData item class from a DataMap.
     */
    public interface Creator<T> extends Parcelable.Creator<T>
    {
         ...
         * {@link com.aa.android.common.data.SharedData#writeToDataMap(com.google.android.gms.wearable.DataMap)}.
         *
         * @param source
         ...
         */
        public T createFromDataMap(DataMap source);
    }
}
